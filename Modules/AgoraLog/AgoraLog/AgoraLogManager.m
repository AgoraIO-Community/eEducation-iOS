...
#define AgoraLogError(level, context, frmt, ...)   LOG_MAYBE(NO, level, DDLogFlagError,   context, nil, __PRETTY_FUNCTION__, frmt, ##__VA_ARGS__)
...
//    }
    ...
    DDContextWhitelistFilterLogFormatter *formatter = [[DDContextWhitelistFilterLogFormatter alloc] init];
...
- (AgoraLogErrorType)uploadLogWithOptions:(AgoraLogUploadOptions*)options progress:(void (^ _Nullable) (float progress))progressBlock success:(void (^ _Nullable) (NSString *serialNumber))successBlock failure:(void (^ _Nullable) (NSError *error))failureBlock {
    ...
    [LogHttpClient setAgoraLogManager:self];
