...
+ (instancetype)shareManager{
    @synchronized(self){
        if (!manager) {
            ...
            [AgoraLogService shareManager].logManager = [AgoraLogManager new];
        }
        return manager;
    }
}
...
+ (void)uploadDebugItem:(EduDebugItem)item appId:(NSString *)appId success:(void (^)(NSString *serialNumber)) successBlock failure:(void (^)(NSError *error))failureBlock {
