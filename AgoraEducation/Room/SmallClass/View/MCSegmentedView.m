...
- (void)setSegmentType:(SegmentType)segmentType {
    if (segmentType == SegmentTypeList) {
        self.items = [NSArray arrayWithObjects:NSLocalizedString(@"StudentListText", nil), NSLocalizedString(@"ChatroomText", nil), nil];
        
    } else if (segmentType == SegmentTypeMessage) {
        self.items = [NSArray arrayWithObjects:NSLocalizedString(@"ChatroomText", nil), NSLocalizedString(@"StudentListText", nil), nil];
    }
    
    for(UIButton *btn in self.subviews) {
        if([btn isKindOfClass:UIButton.class] && btn.tag >= 1000) {
